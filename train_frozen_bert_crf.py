...
def train_seed(seed):
    ...
    optimizer_grouped_parameters = [
        {"params": [p for n, p in model.classifier.named_parameters()
                    if not any(nd in n for nd in no_decay)], "lr": LR_CRF, "weight_decay": 0.01},
        {"params": [p for n, p in model.classifier.named_parameters()
                    if any(nd in n for nd in no_decay)],     "lr": LR_CRF, "weight_decay": 0.0},
    ...
    for epoch in range(EPOCHS):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            torch.save(model.state_dict(), f"best_frozen_bertcrf_seed{seed}.pt")
            print(f"  saved (val f1 {best_val_f1:.4f})")


if __name__ == "__main__":
    for seed in SEEDS:
        print(f"\n=== Frozen BERT+CRF seed {seed} ===")
        train_seed(seed)
